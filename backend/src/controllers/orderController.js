...
const adminCreateShipment = async (req, res) => {
    try {
        ...
        const updatedShipment = await orderService.updateShipmentInfo(id, shipmentData);

        // Tiện tay cập nhật luôn trạng thái đơn hàng sang shipped
        await orderService.updateOrderStatus(id, 'SHIPPED');

        res.status(200).json({ success: true, message: "Đã tạo thông tin giao hàng", data: updatedShipment });
    } catch (error) {
        res.status(400).json({ success: false, message: error.message });
    }
};
