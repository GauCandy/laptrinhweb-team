...
const updateProduct = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, messgae: "Cập nhật thành công", data: product });
  } catch (error) {
    res.status(400).json({ success: false, message: error.message });
  }
};
...
const toggleFeatured = async (req, res) => {
  try {
    // currentStatus là boolean (true/false) Admin gửi lên
    const { currentStatus } = req.body;
    const updateProduct = await productService.toggleFeaturedStatus(
      req.params.id,
      currentStatus,
    );

    const message = updateProduct.isFeatured
      ? "Đã gắn sao nổi bật!"
      : "Đã gỡ sao nổi bật!";
